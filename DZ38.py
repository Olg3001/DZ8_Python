...
def print_records(phones: str):
    with open(phones, 'r', encoding='utf-8') as data:
        for line in data:
            print(*line.split(';'), end='')


def input_records(phones: str):
    with open(phones, 'r+', encoding='utf-8') as data:
        record_id = 0
        for line in data:
            if line != '':
...
def find_records(phones: str, char, condition):
    if condition != 'q':
        printed = False
        with open(phones, 'r', encoding='utf-8') as data:
            for line in data:
...
def check_id_record(phones: str, text: str):
    ...
    if decision == '1':
        ...
        while not find_records(phones, '0', record_id) and record_id != 'q':
            record_id = input('Введите id, q - выйти\n')
        return record_id
    return decision
...
def replace_record_line(phones: str, record_id, replaced_line: str):
    replaced = ''
    with open(phones, 'r', encoding='utf-8') as data:
        for line in data:
            replaced += line
...
def change_records(phones: str):
    record_id = check_id_record(phones, 'изменить')
    if record_id != 'q':
        ...
        if confirm == 'y':
            replace_record_line(phones, record_id, replaced_line)


def delete_records(phones: str):
    record_id = check_id_record(phones, 'удалить')
    if record_id != 'q':
        confirm = confirmation('удаление')
        if confirm == 'y':
            replace_record_line(phones, record_id, '')
